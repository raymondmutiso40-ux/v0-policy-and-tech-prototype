...
interface Report {
  ...
  evidence_files: { pathname: string; filename: string; size: number }[]
...
export default function AdminDashboard() {
  ...
  const [newAuthority, setNewAuthority] = useState({ name: "", email: "", phone: "" })
  ...
  const unreadCount = cases.filter(c => !c.is_read && c.status === "submitted").length
  ...
  const handleAddAuthority = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setNewAuthority({ name: "", email: "", phone: "" })
        fetchAuthorities()
      } else {
        const data = await response.json()
        toast.error(data.error || "Failed to add authority")
      }
    } catch {
  ...
  const handleDeleteAuthority = async (id: string) => {
    if (!confirm("Are you sure you want to remove this authority?")) return

    try {
      const response = await fetch(`/api/admin/authorities?id=${id}`, {
        method: "DELETE",
      })
  ...
  const handleForwardCase = async () => {
    if (!selectedCase || selectedAuthorities.length === 0) {
      toast.error("Please select at least one authority")
      return
    }

    setIsForwarding(true)
    try {
      const response = await fetch("/api/admin/forward-case", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (response.ok) {
        ...
        toast.error(data.error || "Failed to forward case")
      }
    } catch {
  ...
  const toggleAuthoritySelection = (id: string) => {
      ...
      <main className="container mx-auto px-4 py-6">
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
            ...
            <Card className={unreadCount > 0 ? "bg-blue-50 border-blue-200" : ""}>
              <CardContent className="p-4">
                <div className="flex items-center gap-2">
        ...
        {/* Tabs for Cases and Authorities */}
                  ...
                  {!selectedCase ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="border-t pt-4 space-y-3">
                        ...
                        {selectedCase.evidence_files && selectedCase.evidence_files.length > 0 && (
                          <div>
                            <p className="text-xs text-muted-foreground">Uploaded Evidence ({selectedCase.evidence_files.length} files)</p>
                            <div className="mt-1 space-y-1">
                              {selectedCase.evidence_files.map((file, idx) => (
                      ...
                      <div className="border-t pt-4">
                        ...
                        {selectedCase.is_anonymous ? (
                          ...
                          <div className="space-y-2 text-sm">
                            {selectedCase.reporter_name && <p><strong>Name:</strong> {selectedCase.reporter_name}</p>}
                            {selectedCase.reporter_email && <p><strong>Email:</strong> {selectedCase.reporter_email}</p>}
                            {selectedCase.reporter_phone && <p><strong>Phone:</strong> {selectedCase.reporter_phone}</p>}
                          </div>
                        )}
                      </div>
                      ...
                      <div className="border-t pt-4">
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <p className="font-medium">Forward to Authority</p>
                        </div>
                          ...
                          {authorities.map((auth) => (
                            <div key={auth.id} className="flex items-center gap-2">
                              <Checkbox
                                ...
                                onCheckedChange={() => toggleAuthoritySelection(auth.id)}
                              />
                              <label htmlFor={auth.id} className="text-sm cursor-pointer">
                                {auth.name}
                              </label>
                            </div>
                          ))}
                        ...
                        <Button 
                          ...
                          disabled={selectedAuthorities.length === 0 || isForwarding}
                          className="w-full"
                        >
                          {isForwarding ? <Spinner className="mr-2" /> : <Mail className="h-4 w-4 mr-2" />}
                          Forward Case via Email
                        </Button>
                      </div>
                    </div>
                  )}
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Add legal authorities who should receive case notifications
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <form onSubmit={handleAddAuthority} className="space-y-4">
                    <div>
                      ...
                      <Input
                        placeholder="e.g., DCI Cybercrime Unit"
                    ...
                    <div>
                      ...
                      <Input
                        type="email"
                        placeholder="e.g., cybercrime@dci.go.ke"
                    ...
                    <div>
                      ...
                      <Input
                        placeholder="e.g., +254 800 722 203"
                        value={newAuthority.phone}
                        onChange={(e) => setNewAuthority({ ...newAuthority, phone: e.target.value })}
                      />
                    </div>
                    <Button type="submit" className="w-full" disabled={isAddingAuthority}>
                      {isAddingAuthority ? <Spinner className="mr-2" /> : <Plus className="h-4 w-4 mr-2" />}
                      Add Authority
                    </Button>
                  </form>
                </CardContent>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  {authorities.length === 0 ? (
                    <p className="text-center text-muted-foreground py-8">No authorities registered yet</p>
                  ) : (
                    authorities.map((auth) => (
                      <div
                        key={auth.id}
